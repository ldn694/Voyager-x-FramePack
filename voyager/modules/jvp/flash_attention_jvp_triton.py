...
---------------------------------------------------------------------------
VENDORING NOTE (Voyager fork)
---------------------------------------------------------------------------
Vendored verbatim from rcm/rcm/utils/flash_attention_jvp_triton.py with the
block-sparse / causal "Magi" path stripped out, since Voyager's RGB-D i2v
uses dense full attention only. Relative to the upstream file:
  - removed imports of rcm.utils.{magimask,blockmask}
  - removed the `_attn_fwd_magi` kernel + `_magi_configs` autotune set
  - removed the `magi_mask` argument / branch from `_attention.forward`
  - removed `_build_bool_mask_from_spec`, `_test_jvp_magi`, `test_jvp_magi`
The dense `_attn_fwd` kernel and its JVP math are byte-identical to upstream.
"""
