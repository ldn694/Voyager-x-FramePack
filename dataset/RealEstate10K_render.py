...
class RealEstate10K_render(Dataset):
    def __init__(self, 
                 root_path,
                 set_name, 
                 frame_per_sample=49, 
        ...
        for media_file in tqdm.tqdm(self.media_files, desc="Loading media files"):
            ...
            if '_' in os.path.basename(media_file):
                base_name = os.path.basename(media_file).split('_')[0] + '.txt'
                media_file = os.path.join(self.folder_path, base_name)
    ...
    def __getitem__(self, index):
        ...
        #================RENDERING================#
        frames = []
        for i in range(self.frame_per_sample):
            frames.append(Frame(
                rgb=ToPILImage()(images[i]),
                depth=depth_tensor[0, i].numpy(), # Assuming depth_tensor is (1, T, H, W)
                camera=Camera(intrinsics[i].numpy(), w2c[i].numpy()),
                is_reverse_depth=True,
            ))

        images = images.permute(1, 0, 2, 3)      # (3, T, H, W)
        
        # 3. Perform Rendering (Ground Truth)
        # Use a simplified version of your render function that handles a single sample
        result = {
    ...
    def _render_sample(self, frames, shape, partial=False):
        """Helper to render a single sample's frames."""
        ...
        for frame in frames:
            ...
            img_np = np.asarray(rendered_image, dtype=np.float32) / 255.0
            norm_rgbs.append(torch.from_numpy(img_np).permute(2, 0, 1))
            norm_depths.append(torch.from_numpy(np.asarray(depth_buffer, dtype=np.float32)).unsqueeze(0))
            masks.append(torch.from_numpy(np.asarray(mask, dtype=np.float32)).unsqueeze(0))

        # Stack and process to match the training script's expected input
        # Note: You can call your existing load_rgbs_depths/load_masks logic here 
        # or just return the tensors and do the final concat in the training loop.
        # Returning raw stacks for now:
        rgbs_stack = torch.stack(norm_rgbs, dim=1)  # (3, T, H, W)
        depths_stack = torch.stack(norm_depths, dim=1) # (1, T, H, W)
        masks_stack = torch.stack(masks, dim=1) # (1, T, H, W)
        
        return (rgbs_stack, depths_stack), masks_stack
...
if __name__ == "__main__":
    ...
    dataset = RealEstate10K_render(root_path='.', set_name='refined_test_150', transform=transform, width=1280, height=720)
